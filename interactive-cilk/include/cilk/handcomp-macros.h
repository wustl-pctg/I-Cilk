...
void __cilkrts_set_current_sf_future_parent(void);
void __cilkrts_clear_current_sf_future_parent(void); 
}
...
#define START_FUTURE_SPAWN__INTEROP\
  __cilkrts_set_current_sf_future_parent();\
  initial_fiber = cilk_fiber_get_current_fiber();\
  __STACK_SWITCH;
    
#define START_FIRST_FUTURE_SPAWN__INTEROP\
    cilk_fiber* initial_fiber;\
    START_FUTURE_SPAWN__INTEROP;
...
#define END_FUTURE_SPAWN__INTEROP \
  ...
  cilk_fiber_do_post_switch_actions(initial_fiber);\
  __cilkrts_clear_current_sf_future_parent();
...
#define CILK_INITIAL_FUNC_PREAMBLE\
