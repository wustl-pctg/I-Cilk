...
#   define ASSERT_WORKER_LOCK_OWNED(w)                          \
  {                                                             \
    ...
    CILK_ASSERT((w)->l->lock[tls_worker->l->scheduling_level].owner == tls_worker);              \
  }
#   define ASSERT_DEQUE_LOCK_OWNED(d)                           \
  {                                                             \
    __cilkrts_worker *tls_worker = __cilkrts_get_tls_worker();  \
    CILK_ASSERT((d)->lock[tls_worker->l->scheduling_level].owner == tls_worker);                 \
  }
#else
#   define ASSERT_WORKER_LOCK_OWNED(w)
#   define ASSERT_DEQUE_LOCK_OWNED(d)
...
__cilkrts_deque_link* __cilkrts_get_deque_link() {
    c_deque* d = (c_deque*)__cilkrts_get_deque();
    return &(d->link);
}
...
int deque_init(c_deque *d, size_t ltqsize, int scheduling_level)
{
  ...
  d->scheduling_level = scheduling_level;
...
cilk_fiber* deque_suspend(__cilkrts_worker *w, c_deque *new_deque, int make_resumable, int switch_sched_lvl)
{
  c_deque *d = w->l->active_deque;
  int64_t start_time;
  //if (make_resumable) d->resumable = DEQUE__RESUMABLE;
  
  __cilkrts_idle_stats_acquire_own_lock(w);

  start_time = cilk_prio_get_current_time_tsc_ticks();
  int new_deque_level = w->l->next_scheduling_level;
  int old_level = w->l->scheduling_level;
  if (new_deque) {
    CILK_ASSERT(new_deque_level == new_deque->scheduling_level
        || old_level == new_deque->scheduling_level);
    // TODO: Need to look into who passes in a deque deque
    new_deque_level = new_deque->scheduling_level;
  }

  if (new_deque_level != old_level) {
    // Admittedly, this is optimistic; however, if we don't steal successfully
    // (which should happen soon), then we will switch back to idle
    __cilkrts_idle_stats_update_on_sched_level_change_previously_locked(w, &start_time, new_deque_level, IDLE_STATS__ACTIVE_STATE);
  }
  __cilkrts_idle_stats_release_own_lock(w);


  CILK_ASSERT(d->scheduling_level == old_level);
  ...
  if (!new_deque) {
    ...
    __cilkrts_mutex_lock(w, &w->l->lock[new_deque_level]);
    int size = w->l->resumable_deques[new_deque_level].size;
    if (size > 0) {
        #ifdef COLLECT_STEAL_STATS
            w->l->ks_stats.deques_mugged_on_suspend++;
        #endif
      new_deque = w->l->resumable_deques[new_deque_level].array[size-1];
      deque_mug(w, new_deque);
    }
    __cilkrts_mutex_unlock(w, &w->l->lock[new_deque_level]);
    /// @todo{ Check other workers, too? }
  }

  if (!new_deque) { // Must allocate new
    __cilkrts_idle_stats_transition_to_idle(w, start_time);
    new_deque = __cilkrts_allocate_deque();
    if (new_deque
        && -1 != deque_init(new_deque, w->g->ltqsize, new_deque_level)) {
      new_deque->team = d->team;
      new_deque->fiber = w->l->scheduling_fiber;
      attempt_steal = (d->scheduling_level == new_deque_level);
    } else {
    ...
    CILK_ASSERT(__cilkrts_deque_can_be_activated(new_deque));
    if (new_deque->resumable == DEQUE__RESUMABLE) {
      CILK_ASSERT(new_deque->fiber);
      CILK_ASSERT(new_deque->self == INVALID_DEQUE_INDEX);
      CILK_ASSERT(new_deque->worker == NULL);
    } else {
      // NOTE: This is executed on a deque we got from our own resumable pool
      //       it IS possible that the code reaches this point
      //       SET UP A FUNCTION THAT SETS UP THE DEQUE FOR EXECUTION; CALL IT
      //       FROM HERE AND THE RANDOM STEAL PATH!!!
      //CILK_ASSERT(!"SHOULD NOT GET HERE NOW!");
      CILK_ASSERT(new_deque->resumable == DEQUE__RUNNABLE_TASK);
      // TODO: Make sure the fiber allocation, etc., works
      deque_make_runnable_resumable(new_deque);
    }
    new_deque->resumable = DEQUE__NOT_RESUMABLE;
  }
  ...
  __cilkrts_mutex_lock(w, &w->l->lock[old_level]); {
    ...
    w->l->scheduling_level = new_deque_level;
    ...
    // Not sure we need to lock ourselves here
  ...
  if (make_resumable) CILK_ASSERT(d->resumable == DEQUE__RESUMABLE);
  if (d->resumable == DEQUE__RESUMABLE || d->head != d->tail) {
    ...
    if (__builtin_expect(w->g->total_workers > 1, 1)) {
      ...
      if ((__cilkrts_deque_can_be_activated(d) && victim->l->resumable_deques[d->scheduling_level].size > potential_victim->l->resumable_deques[d->scheduling_level].size)
          || (d->resumable == DEQUE__NOT_RESUMABLE && victim->l->suspended_deques[d->scheduling_level].size > potential_victim->l->suspended_deques[d->scheduling_level].size)) {
    ...
    __cilkrts_mutex_lock(w, &victim->l->lock[d->scheduling_level]);
    CILK_ASSERT(d->resumable != DEQUE__RUNNABLE_TASK);
      if (d->resumable == DEQUE__RESUMABLE) {
        deque_pool_add(victim, &victim->l->resumable_deques[d->scheduling_level], d);
      } else if (d->head != d->tail) {
        deque_pool_add(victim, &victim->l->suspended_deques[d->scheduling_level], d);
      }
    __cilkrts_mutex_unlock(w, &victim->l->lock[d->scheduling_level]);
  } else {
    d->self = INVALID_DEQUE_INDEX;
    d->worker = NULL;
  }
  //d->worker = NULL;
  //CILK_ASSERT(d->worker == w);
  //CILK_ASSERT(d->self == ACTIVE_DEQUE_INDEX);
  //d->worker = NULL;
  //__cilkrts_throw_deque(d);
...
void deque_mug(__cilkrts_worker *w, c_deque *d)
{
  ...
  CILK_ASSERT(d->worker->l->lock[d->scheduling_level].owner == w);
  CILK_ASSERT(d->resumable == DEQUE__RUNNABLE_TASK || d->fiber);

  deque_pool *p = (__cilkrts_deque_can_be_activated(d)) ?
    &d->worker->l->resumable_deques[d->scheduling_level] : &d->worker->l->suspended_deques[d->scheduling_level];
  ...
  if (d->call_stack) d->call_stack->worker = w;
}

void deque_make_runnable_resumable(c_deque *d) {
      CILK_ASSERT(d->worker == NULL);
      CILK_ASSERT(d->self == INVALID_DEQUE_INDEX);
      __cilkrts_worker *w = __cilkrts_get_tls_worker_fast();
      CILK_ASSERT(!d->fiber);
      cilk_fiber *f = cilk_fiber_allocate(&w->l->fiber_pool);
      CILK_ASSERT(f);
      cilk_fiber_reset_state(f,
                            fiber_proc_to_execute_task);
      CILK_ASSERT(!d->frame_ff);
      d->frame_ff = __cilkrts_make_full_frame(w, NULL);
      CILK_ASSERT(d->frame_ff);
      d->frame_ff->join_counter++;
      d->frame_ff->fiber_self = f;
      //d->resumable = DEQUE__RESUMABLE;
      d->team = w->g->workers[0];
      __asm__ volatile ("" ::: "memory");
      d->fiber = f;
      // NOTE: saved_ped, link, tails, heads, ltqs, should be set
      //       on creation of the runnable task deque; call_stack should be NULL
}

void __cilkrts_throw_deque(void *void_d) {
  c_deque *const d = (c_deque*) void_d;
  CILK_ASSERT(d->worker == NULL);
  __cilkrts_worker *w = __cilkrts_get_tls_worker_fast();
  // Might be nice to use trylock here, but I'm not sure what is the
  // right thing to do if it fails.
  if (__cilkrts_deque_can_be_activated(d) || d->head != d->tail) {
    ...
    if (__builtin_expect(w->g->total_workers > 1, 1)) {
        ...
        if ((__cilkrts_deque_can_be_activated(d) && victim->l->resumable_deques[d->scheduling_level].size > potential_victim->l->resumable_deques[d->scheduling_level].size) || (d->resumable == DEQUE__NOT_RESUMABLE && victim->l->suspended_deques[d->scheduling_level].size > potential_victim->l->suspended_deques[d->scheduling_level].size)) {
          victim = potential_victim;
        }
    } else {
      victim = w->g->workers[0];
    ...
    __cilkrts_mutex_lock(w, &victim->l->lock[d->scheduling_level]); {
      if (__cilkrts_deque_can_be_activated(d)) {
        deque_pool_add(victim, &victim->l->resumable_deques[d->scheduling_level], d);
