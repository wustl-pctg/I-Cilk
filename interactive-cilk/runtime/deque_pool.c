...
{
  p->capacity = cap;
  c_deque **new_array = (c_deque**) __cilkrts_malloc(cap * sizeof(c_deque*));

  if (!new_array)
    __cilkrts_bug("W%d could not resize deque pool to capacity %zu\n",
                  __cilkrts_get_tls_worker()->self, cap);

  memcpy(new_array, p->array, p->size * sizeof(c_deque*));
...
void deque_pool_init(deque_pool *p, size_t ltqsize, int scheduling_level)
{
  p->array = NULL;
  p->size = 0;
  p->scheduling_level = scheduling_level;
  resize(p, ltqsize);
}
...
void __cilkrts_suspend_deque(int make_resumable, int switch_sched_level)
{
  __cilkrts_worker *w = __cilkrts_get_tls_worker_fast();
  ...
  c_deque *prev = w->l->active_deque;

  if (make_resumable) prev->resumable = DEQUE__RESUMABLE;
  // Sets fiber in active deque
  current_fiber = deque_suspend(w, NULL, make_resumable, switch_sched_level);
  ...
  if (w->l->active_deque) {
    ...
    if (w->l->active_deque->fiber != w->l->scheduling_fiber) {
      while (!deque_is_runnable(w->l->active_deque)
             && (!w->l->active_deque->fiber
...
void __cilkrts_make_resumable(void* _deque)
{
  ...
  c_deque *deque_to_resume = (c_deque*) _deque;

  // They could still be suspending!!!
  //while (deque_to_resume->resumable != DEQUE__NOT_RESUMABLE);
  CILK_ASSERT(deque_to_resume->resumable == DEQUE__NOT_RESUMABLE);

  // At this point, no one else can resume the deque.
  // Wait for the deque to be fully suspended.
  while (!deque_to_resume->fiber
         || ((cilk_fiber_get_current_fiber() != deque_to_resume->fiber) && !cilk_fiber_is_resumable(deque_to_resume->fiber)));

  int previously_owned = 0;
  __cilkrts_worker volatile* victim = deque_to_resume->worker;
  if (victim) { 

    __cilkrts_mutex_lock(w, &victim->l->lock[deque_to_resume->scheduling_level]); {
      if (deque_to_resume->worker) { 
        CILK_ASSERT(deque_to_resume->worker == victim);
        previously_owned = 1;
        deque_pool_remove(&victim->l->suspended_deques[deque_to_resume->scheduling_level], deque_to_resume);
        CILK_ASSERT(deque_to_resume->self == INVALID_DEQUE_INDEX);
        deque_to_resume->resumable = DEQUE__RESUMABLE;
        deque_pool_add((__cilkrts_worker*)victim, &victim->l->resumable_deques[deque_to_resume->scheduling_level], deque_to_resume);
      }
      
    } __cilkrts_mutex_unlock(w, &victim->l->lock[deque_to_resume->scheduling_level]);
  }
  //CILK_ASSERT(deque_to_resume->self == INVALID_DEQUE_INDEX);
  ...
  if (previously_owned) {
    ...
    //CILK_ASSERT(deque_to_resume->worker == NULL);
    //deque_to_resume->resumable = DEQUE__RESUMABLE;
    //__cilkrts_throw_deque(deque_to_resume);
    ...
    if (w->g->total_workers > 1) {
      ...
      if (victim->l->resumable_deques[deque_to_resume->scheduling_level].size > potential_victim->l->resumable_deques[deque_to_resume->scheduling_level].size) {
    ...
    CILK_ASSERT(!previously_owned);
    CILK_ASSERT(deque_to_resume->self == INVALID_DEQUE_INDEX);
    deque_to_resume->resumable = DEQUE__RESUMABLE;
    __cilkrts_mutex_lock(w, &victim->l->lock[deque_to_resume->scheduling_level]);
    deque_pool_add(victim, &victim->l->resumable_deques[deque_to_resume->scheduling_level], deque_to_resume);
    __cilkrts_mutex_unlock(w, &victim->l->lock[deque_to_resume->scheduling_level]);
...
void __cilkrts_resume_suspended(void* _deque, int enable_resume)
{
  ...
  if (w->l->type == WORKER_IO || w->current_stack_frame->call_parent || w->l->scheduling_level != deque_to_resume->scheduling_level) {
    __cilkrts_make_resumable(_deque);
    return;
  }

  CILK_ASSERT(w->l->scheduling_level == deque_to_resume->scheduling_level);
  // Could be a race between suspending and resuming!
  //while (deque_to_resume->resumable != DEQUE__NOT_RESUMABLE);
  ...
  if (enable_resume == 2) { // && !w->current_stack_frame->call_parent && !(*w->l->frame_ff)->parent && !(w->current_stack_frame->flags & CILK_FRAME_LAST)) {
    ...
    deque_to_resume->resumable = DEQUE__NOT_RESUMABLE;
    c_deque *to_destroy = w->l->active_deque;
    ...
    deque_to_resume->resumable = DEQUE__NOT_RESUMABLE;
    // TODO: Don't do this anymore, do it in suspend
    w->l->active_deque->resumable = (enable_resume) ? DEQUE__RESUMABLE : DEQUE__NOT_RESUMABLE;
    cilk_fiber *current_fiber = deque_suspend(w, deque_to_resume, enable_resume, 0);
...
void deque_pool_add(__cilkrts_worker *victim, deque_pool *p, c_deque *d)
{
  ...
  CILK_ASSERT(victim->l->lock[d->scheduling_level].owner == w);
  CILK_ASSERT(d->scheduling_level == p->scheduling_level);
  if (__cilkrts_deque_can_be_activated(d))
    CILK_ASSERT(&victim->l->resumable_deques[d->scheduling_level] == p);
  else 
    CILK_ASSERT(&victim->l->suspended_deques[d->scheduling_level] == p);
...
{
  CILK_ASSERT(w->l->lock[p->scheduling_level].owner != NULL);
