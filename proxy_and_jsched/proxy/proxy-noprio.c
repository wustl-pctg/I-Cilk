...
static ArgDefs argp = { args, "Proxy Nopriority", Version, NULL };
...
//double printTable(HashTable* head, int last, int verbose);
...
int calcDig(int num);//get digits of an int
...
int open_clientfd_disk(char *hostname, int is_https); //open disk file with site info
suseconds_t difftimevals(struct timespec t1, struct timespec t2);//get diff time between two timespecs (us)
...
char* webTransfer(volatile int clientfd, volatile int conNum, char* uri, char* request, parseInfo* info, char* bufForCache, struct timespec recTime, cilk::future<char*>* h);//get info from URL and send back to client
char* diskTransfer(volatile int clientfd, volatile int conNum, char* uri, char* request, parseInfo* info, char* bufForCache, struct timespec recTime, cacheBlock* first_hit, cilk::future<char*>* h);//get info from URL and send back to client
int handleConnection(volatile int clientfd, volatile int conNum);//handle a client wrapper
int handleConnection_kernel(volatile int clientfd,volatile int conNum,reqHead_t* head);//handle a clients requests
int getInitialRequest(volatile int clientfd, char* initialRequest, volatile int conNum,reqHead_t* head, struct timespec* recTime, int take_time);//get a request from a client
int sendHeader(int serverfd,volatile int clientfd, char* headerToSend);//send a given header to server
int sendHeaders(int serverfd,volatile int clientfd, char* request, headerStruct headers,struct timespec recTime); //send all headers to server
int writeError(volatile int clientfd, struct timespec recTime, int wb_error);//handle error (write back error to client)
int serverToClient(volatile int clientfd,int serverfd,char* uri,char* bufForCache,sizesCache* sizes,volatile int conNum, struct timespec recTime); //read from URL server and write back to client
int diskToClient(volatile int clientfd,int serverfd,char* uri,char* bufForCache,sizesCache* sizes,volatile int conNum, struct timespec recTime); //read from URL server and write back to client
int tryAndReadFromCache(volatile int clientfd,char* uri,volatile int conNum, struct timespec recTime, cacheBlock** first_hit);//check cache and if hit read back to client
#ifdef FUT_HIT_WB
void cacheRespWriteBack(int clientfd, char* cacheResponse, int returnSize, struct timespec recTime, cilk::future<void>* h); //write back for cache hits
#endif
cacheBlock** collectStats(int calls);//function called by doStats to actually collect
int doStats(int first);//background task to collect stats periodically
double printTable(volatile HashTable* head,int last);//print cache
int resize(volatile HashTable* head,volatile SubTable* ht,cilk::future<int>* r_fut);//moves items from small tables to larger ones in cache
int insertTable(volatile HashTable* head,  volatile unsigned long start, cacheBlock* ent);//insert a cacheblock into cache
int letConnect(char* port);//gets all the client connection and starts their handle connection
int getAll(volatile int conNum, int temp);//get all outstanding futures
cilk::future<cacheBlock**>* s_fut;
...
cilk::future<char*> **web_futs;
#ifdef FUT_HIT_WB
cilk::future<void> **hit_wb_futs; //futures for cache writeback
#endif
...
//primary method which starts up the program with its initiate calls to function
//and definition of local variables
int main(int argc, char** argv) {
  ...
  if(test&&real){
    fprintf(stderr,"Cant have both real and test!\n");
  }
  ...
  web_futs=(cilk::future<char*> **)calloc(MAX_INC, sizeof(cilk::future<char*>*));
#ifdef FUT_HIT_WB
  hit_wb_futs=(cilk::future<void> **)calloc(MAX_INC, sizeof(cilk::future<void>*));
#endif
  //just so can use futures
  //cilk_spawn srand(time(NULL));
  if(rseed){
    srand(rseed);
  }
  else{
  ...
  //make sure cmd arguments are proper

  //if specifying a starting cache size (defaults to 8)
  ...
  print_csv(statTimes, stat_count, "Stat Timing (avg of %d)");
  ...
  print_csvDbl(stats_dbl, stat_count, "Stat Timing (avg of %d) [NORMALIZED]");
  fflush(stdout);
  return 0;
  CILK_FUNC_EPILOGUE;
  //  return 0;
...
int letConnect(char* port){
  ...
  while(cons>maxCon){
    ...
    int h_ret=0;
    cilk_spawn3(h_ret,handleConnection,clientfd, conNum);
  }
  cilk_sync2();
  return 0;
}   
...
void cacheRespWriteBack(int clientfd, char* cacheResponse, int returnSize, struct timespec recTime, cilk::future<void>* h){
  if(h){
    cilk_future_get(h);
  }
  if(real){
...
//info: request type/method
//bufForCache: buffer storing all data for the webpage
//h: previous call
char* webTransfer(volatile int clientfd, volatile int conNum, char* uri, char* request, parseInfo* info,   char* bufForCache, struct timespec recTime, cilk::future<char*>* h){


  if(h){
    char* p_ret=cilk_future_get(h);
    if(p_ret){
      free(p_ret);
    }
  ...
  int hres=sendHeaders(serverfd, clientfd,
		       request, headers, recTime);
  if(hres){
    writeError(clientfd, recTime, 0);
    return request;
  }
  //read web response and relay it to client. If size is small enough add to cache
  //  memset(bufForCache, 0, MAX_OBJECT_SIZE);
  int ret = serverToClient(clientfd, serverfd,uri, bufForCache,&sizes,conNum, recTime);
  if(ret<SUCCESS){
    fprintf(stderr,"Error transmitting info from server to client for %s\n", uri);
    close(serverfd);
    writeError(clientfd, recTime, ret);
    return request;
  }
...
//bufForCache: buffer storing all data for the webpage
//h: previous call
char* diskTransfer(volatile int clientfd, volatile int conNum, char* uri, char* request, parseInfo* info, char* bufForCache, struct timespec recTime, cacheBlock* first_hit, cilk::future<char*>* h){
  ...
  int ret = diskToClient(clientfd, serverfd,uri, bufForCache,&sizes,conNum, recTime);
  if(ret<SUCCESS){
    fprintf(stderr,"Error transmitting info from server to client for %s\n", uri);
    close(serverfd);
    writeError(clientfd, recTime, ret);
...
int handleConnection(volatile int clientfd,volatile  int conNum){
  int return_val;
  ...
  head->tail=NULL;
  head->head=NULL;
  return_val=handleConnection_kernel(clientfd, conNum, head);
  ...
  if(return_val==endLoop){
    ...
    cilk::future<int>* gettemp=new cilk::future<int>();
    cilk_future_create(gettemp, getAll, conNum, tempMC);
...
int getAll(volatile int conNum, int temp){
  if(!temp){
    if(global->r_fut){
      int ret=cilk_future_get(global->r_fut);
      if(!ret){
...
int handleConnection_kernel(volatile int clientfd,volatile int conNum,reqHead_t* head){
  ...
  while(1){
    ...
    if(getInitialRequest(clientfd, initialRequest,conNum, head, &recTime, take_time)==-1){
      return endLoop;
    }
    take_time = 0;
	...
	int res = tryAndReadFromCache(clientfd, uri, conNum, recTime, &first_hit);
	if(res == -1){
	  free(request);
	}
	else{
	  cilk::future<char*>* temp= new cilk::future<char*>();
#ifdef USE_WEB
	  cilk_future_create(temp, webTransfer,clientfd, conNum, uri, request, info,bufForCache,recTime, web_futs[conNum]);
#endif //USE_WEB
#ifdef USE_DISK
	  cilk_future_create(temp, diskTransfer,clientfd, conNum, uri, request, info,bufForCache,recTime, first_hit, web_futs[conNum]);
#endif //USE_DISK
...
//get digits for an int
//num: int to calc digits from
int calcDig(int num){
  int offset=0;
  while(num){
    num=num/10;
...
//head: head of LL
int getInitialRequest(volatile int clientfd, char* initialRequest, volatile int conNum,reqHead_t* head, struct timespec* recTime, int take_time){
...
//clientfd: client to write to
int writeError(volatile int clientfd, struct timespec recTime, int wb_error){
  resp error_resp;
...
int sendHeader(int serverfd,volatile int clientfd, char* headerToSend){
...
int sendHeaders(int serverfd,volatile int clientfd, char* request, headerStruct headers,struct timespec recTime){

  
  //sending the predefined headers to the server
  if(sendHeader(serverfd, clientfd, request)){
    writeError(clientfd, recTime, 0);
    return -1;
  }
  if(sendHeader(serverfd, clientfd, headers.hostHeader)){
    writeError(clientfd, recTime, 0);
    return -1;
  }
  if(sendHeader(serverfd, clientfd, headers.UsrAgntHeader)){
    writeError(clientfd, recTime, 0);
    return 1;
  }
  if(sendHeader(serverfd, clientfd, headers.connHeader)){
    writeError(clientfd, recTime, 0);
    return -1;
  }
  if(sendHeader(serverfd, clientfd, headers.proxConnHeader)){
    writeError(clientfd, recTime, 0);
    return -1;
  }
  return 0;
...
//conNum: local conNum for client
int diskToClient(volatile int clientfd,int serverfd,char* uri,char* bufForCache,sizesCache* sizes,volatile int conNum,struct timespec recTime){
  ...
  if(verbose){
    fprintf(stderr,"Getting Web Data From: %s\n", uri);
  }
#ifdef REAL_IO
  ...
  if(real||test){
    if(verbose){
      fprintf(stderr,"Sending back[Miss]: %s -> { %u , %u==%u , %lu }\n\n",
	      uri,
	      server_resp.len,
	      server_resp.result,
	      PROXY_MISS,
  ...
  if(sizes->check<MAX_OBJECT_SIZE){
    ...
    insertTable(global, global->start, toadd);
...
//sizescache: fields to store different info on buf size/amt read/etc..
//conNum: local conNum for client
int serverToClient(volatile int clientfd,int serverfd,char* uri,char* bufForCache,sizesCache* sizes,volatile int conNum,struct timespec recTime){
...
int tryAndReadFromCache(volatile int clientfd,char* uri,volatile int conNum, struct timespec recTime, cacheBlock** first_hit){
#ifdef UTIL_RESET
...
#ifdef FUT_HIT_WB
    cilk::future<void>* temp = new cilk::future<void>();
    cilk_future_create(temp,
		       cacheRespWriteBack,
		       clientfd,
		       cacheResponse,
		       returnSize,
...
void recursive(cacheBlock** arr, int start, int end){
  if(end>start){
    ...
    cilk_spawn3_void(recursive,arr, start, p);
...
cacheBlock** collectStats(int calls){
...
int doStats(int first){
  ...
  while( (res = cilk_read_sync(timerfd, (char*)&expirations, sizeof(expirations)))){
    ...
    if(!first){
      cacheBlock** arr=cilk_future_get(s_fut);
    ...
    first=0;
    cilk::future<cacheBlock**>* temp= new cilk::future<cacheBlock**>();
    s_fut=temp;
    clock_gettime(CLOCK_MONOTONIC,&statStart[calls]);
    cilk_future_create(s_fut, collectStats, calls);
...
//head: head of hashtable
double printTable(volatile HashTable* head,int last){
  volatile SubTable* ht=NULL;
  double count=0;
  double totalSize=0;
  int * items=NULL;
...
int resize(volatile HashTable* head,volatile SubTable* ht,cilk::future<int>* r_fut){
  if(r_fut){ 
    int r_ret=cilk_future_get(r_fut); 
    if(!r_ret){
      printf("Error Resizing\n"); 
    } 
  }
  for(int i =0;i<ht->TableSize;i++){ 
    ...
    if(getPtr(ht->InnerTable[i])){ 
      insertTable(head, head->start+1, getPtr(ht->InnerTable[i])); 
    } 
  }  
...
int addDrop(volatile HashTable* head,volatile  SubTable* toadd, unsigned long AddSlot, cacheBlock* ent,volatile unsigned long start){
  ...
  if(res){ 
    ...
    return insertTable(head, start, ent); 
  } 
  else{ 
...
int insertTable(volatile HashTable* head,  volatile unsigned long start, cacheBlock* ent){
  //  int insertTable(HashTable* head,  int start, cacheBlock* ent){ 
  volatile SubTable* ht=NULL; 
  ...
  for(int j=start;j<head->cur;j++){ 
	...
	cilk_future_create(temp, resize, head, ht, head->r_fut);
	head->r_fut=temp;
  ...
  return addDrop(head, new_table, LocalCur, ent, start+1);
